...
// Stock Market class
Investment::StockMarket::StockMarket(string names[5], int prices[5])
{
    totalStock = 5;
    stock.resize(totalStock);
    for (int i = 0; i < totalStock; i++)
    {
        stock.at(i).setStock(names[i], prices[i]);
    }
}
...
void Investment::StockMarket::buy(int i)
{
    int q;
    stock[i].showStock();
    cout << "Enter quantity : ";
    if (cin >> q && (q * stock[i].getPrice()) <= getBalance())
    {
    cin >> q;
        cout << q << endl;
...
void Investment::StockMarket::sell(int i)
{
    int q;
    cout << "Enter quantity : ";
...
void Investment::StockMarket::stockMarket()
{
    int i, ch;
    while (1)
    {
...
void Investment::saveData(DematAccount ac)
{
...
void Investment::loadData(DematAccount &ac)
{
